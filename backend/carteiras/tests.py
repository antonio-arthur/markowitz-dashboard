...
from carteiras.views import AnalisarCarteiraView, AcoesDisponiveisView, gerar_historico_comparativo
...
class AcoesDisponiveisViewTests(TestCase):
    ...
    def test_gerar_historico_comparativo_usa_retorno_simples(self):
        ...
        self.assertEqual(historico['carteira'][0], 100.0)
        self.assertAlmostEqual(historico['carteira'][1], 110.0)
        self.assertAlmostEqual(historico['carteira'][2], 104.5)

    @patch('builtins.open', side_effect=FileNotFoundError)
    def test_endpoint_retorna_fallback_quando_catalogo_nao_existe(self, _mock_open):
        request = self.factory.get('/api/acoes/', {'busca': 'petr'})
        ...
        self.assertEqual(payload[0]['codigo'], 'PETR4')
    ...
    def test_endpoint_retorna_acoes_filtradas_por_busca(self):
        ...
        self.assertEqual(payload[0]['codigo'], 'PETR3')
...
class AnalisarCarteiraViewTests(TestCase):
    ...
    def test_endpoint_agregado_retorna_dados_completos(self, mock_processar_dados, mock_benchmark):
        precos = pd.DataFrame(
            {
        ...
        retorno_medio = pd.Series({'PETR4.SA': 0.12, 'VALE3.SA': 0.15})
        cov_matrix = pd.DataFrame(
            [[0.04, 0.01], [0.01, 0.03]],
            index=['PETR4.SA', 'VALE3.SA'],
            columns=['PETR4.SA', 'VALE3.SA'],
        )

        mock_processar_dados.return_value = (precos, retornos, retorno_medio, cov_matrix, ['PETR4.SA', 'VALE3.SA'])
        mock_benchmark.return_value = pd.Series([0.01, 0.02], index=retornos.index)
        ...
        self.assertEqual(payload['taxa_livre_risco'], 0.10)
    ...
    def test_endpoint_retorna_historico_real_para_carteira_multiactivo(
        ...
        precos = pd.DataFrame(
            {
                'PETR4.SA': np.linspace(100.0, 160.0, 61),
                'VALE3.SA': np.linspace(50.0, 80.0, 61),
            },
            index=index,
        )
        retornos = pd.DataFrame(
            {
                'PETR4.SA': np.full(60, 0.01),
                'VALE3.SA': np.full(60, 0.015),
            },
            index=index[1:],
        )
        retorno_medio = pd.Series({'PETR4.SA': 0.01, 'VALE3.SA': 0.015})
        cov_matrix = pd.DataFrame(
        ...
        mock_benchmark.return_value = pd.Series(np.full(60, 0.005), index=retornos.index)
        mock_otimizar_minima_variancia.return_value = (np.array([0.6, 0.4]), 0.011, 0.008)
        mock_otimizar_maximo_sharpe.return_value = (np.array([0.5, 0.5]), 0.012, 0.007)
        ...
        self.assertEqual(response.status_code, 200)
        payload = response.data
        self.assertFalse(payload['historico']['demonstrativo'])
        self.assertGreater(len(payload['historico']['meses']), 3)
        self.assertGreater(len(payload['historico']['carteira']), 3)
        self.assertGreater(len(payload['historico']['ibovespa']), 3)
